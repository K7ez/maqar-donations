<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display text-xl font-bold text-primary">تعديل بيانات المتبرع</h2>
    </x-slot>

    <form method="POST" action="{{ route('donors.update', $donor) }}" class="bg-surface border border-hairline rounded-xl p-6">
        @csrf
        @method('PUT')
